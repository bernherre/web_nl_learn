...
import { buildGuidedPaths, findGuidedPath, guidedPathNodeId } from './learning-paths.js';
...
function renderVerbs({ resetLimit = false } = {}) {
  ...
  elements.verbAtlasList.innerHTML = visible.length ? visible.map((verb) => {
    const reviewed = hasReviewedVerbMetadata(verb);
    const definitionPreview = reviewed ? verb.meaning : 'Specifieke betekenis volgt na taalcontrole.';
    return `<button class="verb-list-item ${verb.infinitive === state.selectedVerb ? 'active' : ''}" type="button" role="option" aria-selected="${verb.infinitive === state.selectedVerb}" data-verb-infinitive="${escapeHtml(verb.infinitive)}"><span class="verb-list-copy"><strong>${escapeHtml(verb.infinitive)}</strong><small class="verb-list-definition">${escapeHtml(definitionPreview)}</small><small class="verb-list-meta">${escapeHtml(verb.semanticLabel)} · ${escapeHtml(verb.auxiliary)}</small></span><span class="verb-list-badges"><span class="${verb.regularity === 'regelmatig' ? 'regular' : 'irregular'}">${verb.regularity === 'regelmatig' ? 'R' : 'OR'}</span>${verb.separable ? '<span>S</span>' : ''}<span>${escapeHtml(verb.level)}</span></span></button>`;
